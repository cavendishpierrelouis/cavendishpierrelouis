...
(function(){
  ...
  if(!window.cavbotBrain){
    ...
    function load(){
      try{ return JSON.parse(globalThis.__cbLocalStore.getItem(LS_KEY) || '{}') || {}; }catch{ return {}; }
    }
    function save(obj){
      try{ globalThis.__cbLocalStore.setItem(LS_KEY, JSON.stringify(obj||{})); }catch{}
    }
    window.cavbotBrain = {
      _internal: {
        analytics: load(),
  ...
  var operatorPlayer = document.getElementById('operator-player');
  var operatorImposter = document.getElementById('operator-imposter');
  ...
  var btnSound = document.getElementById('sound-toggle');
  var soundStateEl = document.getElementById('sound-state');
  var sliderVolume = document.getElementById('sound-volume');

  var analogBase = document.getElementById('analog-base');
  var analogKnob = document.getElementById('analog-knob');

  if(!pitch || !operatorPlayer || !operatorImposter || !ballEl) return;

  var ballColorOptions = [
    {
      gradient: 'radial-gradient(circle at 30% 30%, #ffd880, #ffb347 55%, #ff7a18 90%)',
      border: 'rgba(255, 255, 255, 0.65)',
      shadow: '0 0 12px rgba(255, 179, 71, 0.6)'
    },
    {
      gradient: 'radial-gradient(circle at 35% 25%, #62f1ff, #1a9adf 60%, #0862d0 90%)',
      border: 'rgba(255, 255, 255, 0.5)',
      shadow: '0 0 10px rgba(78, 168, 255, 0.6)'
    },
    {
      gradient: 'radial-gradient(circle at 35% 25%, #caff4f, #8dd322 60%, #5aa200 90%)',
      border: 'rgba(255, 255, 255, 0.55)',
      shadow: '0 0 12px rgba(173, 255, 92, 0.5)'
    }
  ];

  function randomizeBallColor(){
    var option = randomFrom(ballColorOptions);
    if(!option) return;
    ballEl.style.background = option.gradient;
    ballEl.style.borderColor = option.border;
    ballEl.style.boxShadow = option.shadow;
  }
  ...
  var volumeLevel = 0.88;
  function applyVolumeLevel(value){
    var normalized = clampVolume(value);
  ...
  if(sliderVolume){
    applyVolumeLevel(Number(sliderVolume.value) / 100);
  ...
  function navigateHome(target) {
    var href = target || RESTORE_URL || INDEX_URL;
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'cavbot:arcade:navigate', href: href }, '*');
      }
    } catch (_error) {}
    setTimeout(function () {
      try {
      ...
      catch (_error) { window.location.href = href; }
    }, 80);
  }
  ...
  function scheduleRestoreRedirect(reason){
    ...
    setTimeout(function(){
      try{ navigateHome(RESTORE_URL); }catch{ window.location.href = RESTORE_URL; }
    }, 1400);
  }
  ...
  var audioCtx = null;
  var masterGain = null;
  var enableNoise = false; // disable random noise hits to avoid static
  ...
  function scheduleMusicStep(step, t){
    ...
    if(isHat && enableNoise){
      noiseHit({ time:t, dur:0.03, vol:0.05, hp:7500 });
    }
    if((step % 16 === 4 || step % 16 === 12) && enableNoise){
  ...
  // Render
  function placeOperator(el, x, y){
    el.style.transform = 'translate(' + x.toFixed(2) + 'px,' + y.toFixed(2) + 'px)';
  }

  function render(){
    placeOperator(operatorPlayer, state.youX, state.youY);
    placeOperator(operatorImposter, state.impX, state.impY);
  ...
  if(analogBase){
    analogBase.addEventListener('pointerdown', function(e){
      ...
      try{ analogBase.setPointerCapture(e.pointerId); }catch{}
