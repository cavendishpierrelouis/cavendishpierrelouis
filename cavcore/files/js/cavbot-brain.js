...
(function () {
  ...
  if (typeof window !== 'undefined') {
    if (window.__cavbotBrainGen1Loaded) return;
    window.__cavbotBrainGen1Loaded = true;
  }
    ...
    for (i = 0; i < headNodes.length; i++) {
      ...
      for (j = 0; j < eyeNodes.length; j++) {
        ...
        for (k = 0; k < pupilNodes.length; k++) {
          ...
          // mark as managed by Gen 1 head tracking (prevents double-transform jitter)
          try { pupilEl.setAttribute('data-cavbot-pupil-managed', '1'); } catch (e) {}
    ...
    function start() {
      ...
      function update() {
        ...
        for (const { pupil, inner, baseTransform } of pairs) {
          ...
          let t = `translate(${(nx * amt).toFixed(2)}px, ${(ny * amt).toFixed(2)}px)`;
